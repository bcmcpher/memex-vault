...
This skill does NOT duplicate the glossary prompts in `karpathy-wiki-ingest`, `karpathy-wiki-connect`, or `karpathy-wiki-meeting`. Those capture terms opportunistically during source processing. This skill is for deliberate, systematic glossary work on notes that already exist.
...
Then add `defines:: [[term-name]]` to the source note's `## Connections` section. If no `## Connections` section exists, append one.
